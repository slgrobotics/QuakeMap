...
//   members to do their job.  As such, these two properties are set to false by default,
...
namespace LibSys
{
	...
	public sealed class ThreadPool : WaitHandle
	{
		...
		{
			/*
			LibSys.StatusBar.Trace(string.Format("New thread pool {0} created:", poolName));
			LibSys.StatusBar.Trace(string.Format("  initial thread count:      {0}", initialThreadCount));
			LibSys.StatusBar.Trace(string.Format("  max thread count:          {0}", maxThreadCount));
			LibSys.StatusBar.Trace(string.Format("  new thread trigger:        {0} ms", newThreadTrigger));
			LibSys.StatusBar.Trace(string.Format("  dynamic thread decay time: {0} ms", dynamicThreadDecayTime));
			LibSys.StatusBar.Trace(string.Format("  request queue limit:       {0} entries", requestQueueLimit));
			*/
			...
			this.requestQueue = new Stack(requestQueueLimit < 0 ? 4096 : requestQueueLimit);
		...
		~ThreadPool()
		{
			try 
			{
				LibSys.StatusBar.WriteLine("~ThreadPool() - stopping all...");
				try 
				{
					InternalStop(true, 2000);
					Monitor.Exit(this);			// we have strange non-exiting wait on line 771 (ThreadProc() Monitor.Wait(pool,...))
				} 
				catch {}
				LibSys.StatusBar.WriteLine("~ThreadPool() - stopped all threads.");
			} 
			catch {}
		}
		...
		private bool InternalStop( bool wait, int timeout )
		{
			...
			//lock(this)
			{
				LibSys.StatusBar.WriteLine(string.Format( "[{0}, {1}] Stopping pool (# threads = {2})",
												AppDomain.GetCurrentThreadId(), Thread.CurrentThread.Name, currentThreadCount ));
			...
			if( wait )
			{
				...
				// see http://msdn.microsoft.com/library/default.asp?url=/library/en-us/cpref/html/frlrfsystemthreadingwaithandleclasswaitonetopic3.asp
				bool stopComplete = WaitOne(timeout, true);
				//bool stopComplete = WaitOne(new TimeSpan(0, 0, 30), false);
				if(stopComplete)
				{
					LibSys.StatusBar.WriteLine("ThreadPool: Work method signaled.");
				...
				{
					LibSys.StatusBar.WriteLine("ThreadPool: Timed out waiting for work method to signal.");
				}
		...
		public int CurrentThreadCount { get { return currentThreadCount; } }
		...
		private Stack               requestQueue;
		public int RequestQueueCount { get { return requestQueue.Count; } }
		...
		class ThreadWrapper
		{
			...
			void ThreadProc()
			{
				//LibSys.StatusBar.Trace(string.Format( "[{0}, {1}] Worker thread started",
				//	AppDomain.GetCurrentThreadId(), Thread.CurrentThread.Name ));

				bool done = false;

				while( !done && !Project.goingDown )
				{
					WorkRequest wr = null;
					...
					lock( pool )
					{
						...
						while( !pool.stopInProgress && !timedOut && (pool.requestQueue.Count == 0) && !Project.goingDown )
						{
						...
						// process it.
						//
						if( !pool.stopInProgress && !timedOut && (pool.requestQueue.Count > 0) && !Project.goingDown)
						{
							//wr = (WorkRequest)pool.requestQueue.Dequeue();
							wr = (WorkRequest)pool.requestQueue.Pop();
					...
					if( !done && (wr != null) && !Project.goingDown )
					{
						...
						if( newThread != null )
						{
							//LibSys.StatusBar.Trace(string.Format( "[{0}, {1}] Adding dynamic thread to pool",
							//	AppDomain.GetCurrentThreadId(), Thread.CurrentThread.Name ));
							newThread.Start();
						}
						...
						try
						{
							...
							if( targetProc != null )
							{
								...
								//LibSys.StatusBar.Trace(string.Format( "[{0}, {1}] Worker thread started job ",
								//					AppDomain.GetCurrentThreadId(), Thread.CurrentThread.Name ) + wr.procArg + " queue=" + pool.requestQueue.Count + " threadCount=" + pool.currentThreadCount);
								
								targetProc(wr.procArg, wr.timeStampStarted);

								//TimeSpan timeSpent = DateTime.Now - start;
								//LibSys.StatusBar.Trace(string.Format( "[{0}, {1}] Worker thread finished job ",
								//	AppDomain.GetCurrentThreadId(), Thread.CurrentThread.Name ) + wr.procArg + " in " + timeSpent.Milliseconds + " ms");
							}
							else
							{
								//LibSys.StatusBar.Trace(string.Format( "[{0}, {1}] Worker thread processing (dynamic invoke) job ",
								//					AppDomain.GetCurrentThreadId(), Thread.CurrentThread.Name ) + wr.procArg);
								wr.targetProc.DynamicInvoke(wr.procArgs);
							}
						}
						catch( Exception e )
						{
							LibSys.StatusBar.Error(string.Format("Exception thrown performing callback:\n{0}\n{1}", e.Message, e.StackTrace));
						}
						finally
						{
				...
				//LibSys.StatusBar.Trace(string.Format( "[{0}, {1}] Worker thread exiting pool",
				//	AppDomain.GetCurrentThreadId(), Thread.CurrentThread.Name ));
			}
		}
